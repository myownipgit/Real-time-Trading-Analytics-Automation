        ...
        try:
            with sqlite3.connect(self.analytics_db) as conn:
                ...
                if result and result[0]:
                    ...
                    # If no snapshots exist, get the max trade_id from trades table
    ...
    def update_risk_metrics(self, conn):
        ...
        conn.execute("""
            INSERT OR REPLACE INTO risk_metrics (
                ...
                stop_loss_effectiveness_pct, avg_profit_pct,
                max_loss_pct, last_updated
            )
            SELECT 
    ...
    def update_strategy_performance(self, conn):
        ...
        conn.execute("""
            INSERT OR REPLACE INTO strategy_performance (
                strategy_name, total_trades, win_rate, avg_profit_pct,
                total_profit_abs, consistency_score, expectancy, last_updated
            )
            SELECT 
                ...
                1.0 - (STDEV(profit_pct) / NULLIF(AVG(ABS(profit_pct)), 0)) as consistency_score,
    ...
    def update_pair_analytics(self, conn):
        ...
        conn.execute("""
            ...
            SELECT 
                ...
                AVG(CASE WHEN profit_pct > 0 THEN 1.0 ELSE 0.0 END) as win_rate,
                AVG(trade_duration) as avg_duration_minutes,
                STDEV(profit_pct) as volatility_score,
                CURRENT_TIMESTAMP as last_updated
            FROM trades 
            WHERE is_open = 0 
            GROUP BY pair
